...
do
	...
	do
		...
		local wallComboRunning = false -- 防止task.spawn重叠
		local function updateWallComboSpam()
			...
			if Cfg.WallCombo and wallComboMode == "连发" then
				...
				wallComboConnection = RunService.Heartbeat:Connect(function()
					-- 用task.spawn异步执行，防止重操作堵塞Heartbeat帧
					if wallComboSpamming and not wallComboRunning then
						wallComboRunning = true
						task.spawn(function()
							wallcomboveryud()
							wallComboRunning = false
						end)
					end
				end)
			end
